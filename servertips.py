...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        subtotalLabelQuestion = tk.Label(self.master, text='please type in your subtotal:')
        subtotalLabelQuestion.grid(row=0, column=0)
        self.subtotal = tk.StringVar()
        subtotal_entry = tk.Entry(self.master, width=7, textvariable=self.subtotal)
        subtotal_entry.grid(row=0, column=1)
        subtotalLabel = tk.Label(self.master, text='$')
        subtotalLabel.grid(row=0, column=2)

        #nonCashTips entry labels and feild
        nonCashTipsQuestion = tk.Label(self.master, text='please type in your tips excluding cash tips:')
        nonCashTipsQuestion.grid(row=1, column=0)
        self.nonCashTips = tk.StringVar()
        nonCashTips_entry = tk.Entry(self.master, width=7, textvariable=self.nonCashTips)
        nonCashTips_entry.grid(row=1, column=1)
        nonCashTipsLabel = tk.Label(self.master, text='$')
        nonCashTipsLabel.grid(row=1, column=2)

        #cashTips entry labels and feild
        cashTipsQuestion = tk.Label(self.master, text='please type in your cashtips:')
        cashTipsQuestion.grid(row=2, column=0)
        self.cashTips = tk.StringVar()
        cashTips_entry = tk.Entry(self.master, width=7, textvariable=self.cashTips)
        cashTips_entry.grid(row=2, column=1)
        cashTipsLabel = tk.Label(self.master, text='$')
        cashTipsLabel.grid(row=2, column=2)

        #Subtotal entry labels and feild
        userinputQuestion = tk.Label(self.master, text='Were you Habachi or Dine in')
        userinputQuestion.grid(row=3, column=0)
        self.userinput = tk.StringVar()
        self.userinput.set("Dine in") # initial value
        option = tk.OptionMenu(self.master, self.userinput, "Dine in", "Habachi")
        option.grid(row=3, column=1)
        

        startButton = tk.Button(self.master, text="Calculate", command=self.calcTakeHome)
        startButton.grid(column=0, row=4)

        self.totalToBusBoy = tk.StringVar()
        tk.Label(self.master, textvariable=self.totalToBusBoy).grid(column=1, row=5,)
        tk.Label(self.master, text="Total to bus boy:").grid(column=0, row=5,)

        self.totalToChef = tk.StringVar()
        tk.Label(self.master, textvariable=self.totalToChef).grid(column=1, row=6,)
        tk.Label(self.master, text="Total to Chef:").grid(column=0, row=6,)

        self.totalCreditCardFee = tk.StringVar()
        tk.Label(self.master, textvariable=self.totalCreditCardFee).grid(column=1, row=7,)
        tk.Label(self.master, text="Credit Card Fee:").grid(column=0, row=7,)

        self.totalTipsYouGet = tk.StringVar()
        tk.Label(self.master, textvariable=self.totalTipsYouGet).grid(column=1, row=8,)
        tk.Label(self.master, text="Tips you get:").grid(column=0, row=8,)

        self.totalTake = tk.StringVar()
        tk.Label(self.master, textvariable=self.totalTake).grid(column=1, row=9,)
        tk.Label(self.master, text="Total Take Home:").grid(column=0, row=9,)
